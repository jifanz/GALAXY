...
class data_train:
    ...
    def __init__(self, training_dataset, net, args):
        ...
        if 'criterion' not in args:
            self.args['criterion'] = F.cross_entropy
    ...
    def _train(self, epoch, loader_tr, optimizer, weight=None):
        ...
        for batch_id, (x, y, idxs) in enumerate(loader_tr):
            ...
            optimizer.zero_grad()
            out = self.clf(x)
            loss = criterion(out, y.long(), weight=weight.to(device=self.device) if weight is not None else None)
    ...
    def train(self, gradient_weights=None, weight=None):
        ...
        if self.args['optimizer'] == 'sgd':
            ...
            optimizer = optim.Adam(self.clf.parameters(), lr = self.args['lr'], weight_decay=5e-5)
        ...
        while (accCurrent < self.args['max_accuracy']) and (epoch < n_epoch) and (not is_saturated):
            ...
            if gradient_weights is None:
                accCurrent, lossCurrent = self._train(epoch, loader_tr, optimizer, weight=weight)
            else:
