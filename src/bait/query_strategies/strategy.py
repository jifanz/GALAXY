...
class Strategy:
    ...
    def _train(self, epoch, loader_tr, optimizer, weight=None):
        self.clf.train()
        accFinal = 0.
        for batch_idx, (x, y, idxs) in enumerate(loader_tr):
            ...
            optimizer.zero_grad()
            out, e1 = self.clf(x)
            loss = F.cross_entropy(out, y, weight=weight.cuda() if weight is not None else None)
    ...
    def train(self, reset=True, optimizer=0, verbose=True, data=[], net=[]):
        ...
        if type(optimizer) == int: optimizer = optim.Adam(self.clf.parameters(), lr=self.args['lr'], weight_decay=5e-5)
        ...
        labels = torch.Tensor(self.Y.numpy()[idxs_train]).long()
        num_preds = torch.sum((labels.unsqueeze(-1) == torch.arange(self.nClasses)).float(), dim=0)
        if len(data) > 0:
        ...
        bestAcc = 0.
        attempts = 0
        print(1. / torch.clip(num_preds, min=1e-6))
        for _ in range(500):
            accCurrent, lossCurrent = self._train(epoch, loader_tr, optimizer, weight=1. / torch.clip(num_preds, min=1e-6))
