...
for run_name in ["jifan/medmnist"]:
    n_class = 2
    ...
    for run in runs:
        ...
        for alg, run_alg in zip(alg_list, run_alg_names):
            if run_alg in run.name and "Batch Size 100" in run.name:
                idxs = num_queries <= 5000
                runs_dict[alg][0].append(num_queries[idxs])
                runs_dict[alg][1].append(num_minority_queries[idxs])
                runs_dict[alg][2].append(balanced_accuracies[idxs])
