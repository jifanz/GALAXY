...
if __name__ == "__main__":
    wandb.init(project="%s" % data, entity=wandb_name,
               name="Confidence Sampling, Pretrained, Weighted, Batch Size %d, ResNet" % batch_size)
    num_trials = 1
    wandb.config = {
    ...
    for i in range(num_iters - 1):
        ...
        uncertain_idxs = np.argsort(torch.max(pred, dim=-1)[0].cpu().numpy())
    ...
    torch.save(trainer.model, open("confidence_sampling_%s_%d_%d.model" % (data, batch_size, seed), "wb"))
