...
for run_name in ["jifan/cifar100_unbalanced_10"]:
    ...
    for run in runs:
        ...
        for alg, run_alg in zip(alg_list, run_alg_names):
            if run_alg in run.name and "Batch Size 1000" in run.name:
                runs_dict[alg][0].append(num_queries[num_queries <= 30000])
                runs_dict[alg][1].append(num_minority_queries[num_queries <= 30000])
                runs_dict[alg][2].append(balanced_accuracies[num_queries <= 30000])
    ...
    for alg, style, color in zip(alg_list, line_styles, colors):
        ...
        if alg == "Random":
            y_lim = balanced_accuracies_mean[0] - .05
    ...
    for alg, style, color in zip(alg_list, line_styles, colors):
        ...
        num_queries, num_minority_queries, balanced_accuracies = np.array(num_queries), np.array(
            num_minority_queries), np.array(balanced_accuracies)
        num_queries, num_minority_queries, balanced_accuracies = num_queries[:, num_queries[0] <= 30000], \
                                                                 num_minority_queries[:, num_queries[0] <= 30000], \
                                                                 balanced_accuracies[:, num_queries[0] <= 30000]
        num_queries = num_queries[0]
        num_minority_queries_mean = np.mean(num_minority_queries, axis=0)
        num_minority_queries_ste = np.std(num_minority_queries, ddof=1, axis=0) / np.sqrt(num_minority_queries.shape[0])
        balanced_accuracies_mean = np.mean(balanced_accuracies, axis=0)
